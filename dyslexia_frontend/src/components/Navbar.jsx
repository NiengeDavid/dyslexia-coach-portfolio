    ...
    <div>
        <Navbar className={`fixed max-w-screen-xl w-full z-20 top-0 start-0 transition duration-300 ${scrolled ? 'bg-[#006273] bg-opacity-75 rounded-b-xl' : 'bg-transparent'}` }>
            ...
            <div className="flex md:order-2">
                {bioFeed.map((item, index) =>  (
                    <a key={index} href={item.button} className="text-[#25d366] cursor-pointer mr-1 bg-[#075e54] hover:bg-[#128c7e] focus:ring-4 focus:outline-none focus:ring-[#25d366]/50 font-medium rounded-lg text-[6px] px-2 py-1 text-center md:py-2 md:px-4 md:text-sm md:mr-0">WhatsApp💬</a>
                ))}
